...
import SteamworksSDK, { MicroTxnAuthorizationResponseEvent } from '../../src/index';

const APP_ID = Number(process.env.STEAM_APP_ID || 480);
const WAIT_MS = 5 * 60 * 1000;

function testMicroTxnAuthorizationResponse(): void {
  ...
  let received = 0;
  const unsubscribe = steam.user.onMicroTxnAuthorizationResponse((event: MicroTxnAuthorizationResponseEvent) => {
    received++;
