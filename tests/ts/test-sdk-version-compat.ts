 ...
 *
 * Usage:
 *   ts-node tests/ts/test-sdk-version-compat.ts
 *   ts-node tests/ts/test-sdk-version-compat.ts --sdk=test-sdks/1.64
 */

import * as fs from 'fs';
import * as path from 'path';
import * as koffi from 'koffi';
import { INTERFACE_ACCESSOR_PREFIXES, findNewestVersionedSymbol } from '../../src/internal/SteamLibraryLoader';

type SdkCheckResult =
  | { ok: true; libPath: string; results: Record<string, string | null> }
  | { ok: false; error: string };

function resolveLibPath(sdkRoot: string): string {
...
function checkSdk(sdkRoot: string): SdkCheckResult {
