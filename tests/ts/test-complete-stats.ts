/**
 * Test script for Steam Stats API (TypeScript)
 * Tests all stats operations including get/set, user stats, and global stats
 */

/**
 * Comprehensive test covering all 14 Steam Stats API functions (TypeScript)
 * This test demonstrates the complete Steamworks FFI stats functionality
 */

// Import directly from source for development
import SteamworksSDK from '../../src/index';

async function testStatsAPI(): Promise<void> {
  console.log('🧪 Starting Steam Stats API Test (TypeScript)\n');
  ...
  if (globalStatsRequested) {
    ...
    if (historyIntData && historyIntData.history.length > 0) {
      ...
      historyIntData.history.forEach((value: number, index: number) => {
    ...
    if (historyDoubleData && historyDoubleData.history.length > 0) {
      ...
      historyDoubleData.history.forEach((value: number, index: number) => {
        const day = index === 0 ? 'today' : `${index} day(s) ago`;
