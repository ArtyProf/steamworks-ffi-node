...
async function testUserAPI() {
  ...
  try {
    ...
    if (webTicket.success) {
      ...
      if (apiKey && apiKey !== 'YOUR_API_KEY_HERE') {
        ...
        console.log('   - Or pass API key as argument: node test-user.js YOUR_KEY');
  ...
  if (authTicket.success && authTicket.ticketData) {
    ...
    // Map enum values to names for display
    const beginResultNames = ['OK', 'InvalidTicket', 'DuplicateRequest', 'InvalidVersion', 'GameMismatch', 'ExpiredTicket'];
    console.log(`   - Result: ${beginResultNames[beginResult.result] || beginResult.result}`);
    ...
    if (beginResult.success) {
      ...
      const licenseResultNames = ['HasLicense', 'DoesNotHaveLicense', 'NoAuth'];
      console.log(`   - License Result: ${licenseResultNames[licenseResult] || licenseResult}`);
  ...
  const voiceResultNames = ['OK', 'NotInitialized', 'NotRecording', 'NoData', 'BufferTooSmall', 'DataCorrupted', 'Restricted', 'UnsupportedCodec', 'ReceiverOutOfDate', 'ReceiverDidNotAnswer'];
  console.log(`   - Result: ${voiceResultNames[available.result] || available.result}`);
